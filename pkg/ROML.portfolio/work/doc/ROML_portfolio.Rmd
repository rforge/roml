...
## Installation
There are currently some building issues on `R-Forge` therefore the newest versions are not 
available via `"http://R-Forge.R-project.org"` but `"http://roml.R-Forge.R-project.org"`.   
`ROML` and `ROML.portfolio` depend on `ROI (>= 0.2-0)` which is currently only available on
`R-Forge`. The code below shows how to install `ROML` and `ROML.portfolio`, please note this 
...
if ( as.numeric(R.Version()$minor) >= 3.1 ) {
  ...
  download.file(url, (dest <- file.path(tempdir(), "slam.tar.gz")))
...
install.packages("ROML.portfolio", repos="http://roml.R-Forge.R-project.org")
```
